...
**Live agency-self marketing site:** `clients/baragency/` (display brand **BAR Agency**; folder/package slug `baragency`, URL/email use the `bar-agency` form) deployed at https://bar-agency.vercel.app (Tier 2 Astro · trilingual EN/DE/pt-BR · dark "Berlin night" register · contact form Resend-ready but inactive). **Inbound-funnel sprint 2026-06-04** (`icreateyoursite.com` benchmark): `/pricing`, paid `/website-check` micro-product, two free lead-tools (`/tools/website-scan` PSI scanner + `/tools/gbp-check`), 24 German `/webdesign-berlin/*` local-SEO pages, home social-proof/promises/FAQ, WhatsApp/phone wired-but-hidden. **Rebrand + overhaul 2026-06-09:** rebranded to BAR Agency (new favicon/OG); 4-offering catalog (Web Design · SEO and Local Listing · E-Commerce and Business Email · AI Solutions/Booking); pricing add-ons (biz email +€12/mo, languages DE+EN standard + €36/mo per-language, via ⓘ tooltips) + content-update limits; **`/contract` § 2 sourced from `/pricing`** (single source of truth; `SERVICE_SCOPES` deleted; draft banner removed); About reworked; `/tools` in nav; portfolio Bellini swap; real address + Kleinunternehmer § 19 UStG resolved. For now a **permanent noindex demo** at `bar-agency.vercel.app` — no custom domain, real email, or env vars; the `/contact` + `/tools/gbp-check` forms validate but don't send. Lawyer sign-off + funnel content DRAFTs still pending. Per-client docs (2026-06-09→12 section) at `docs/clients/baragency/`. **Berlin-night redesign 2026-06-13→14:** the whole site was re-skinned to a dark "Berlin night" register (WebGL aurora heroes + GSAP motion; legal pages stay light) — **committed (`6b79b92` re-skin + `5e253df` docs) and deployed 2026-06-14 to the `bar-agency.vercel.app` Vercel project** (noindex). The previous Vercel project was deleted; `bar-agency` is now GitHub-connected (auto-deploys `main`; Root Directory = `clients/baragency`). Plan + audit at `docs/audit/REDESIGN-ROLLOUT-PLAN-2026-06-13.md` (+ `…-AUDIT-…`); register documented in `docs/clients/baragency/design.md` § "Berlin night". **Pricing + contract upgrade 2026-06-16→17:** `/pricing` gained a **monthly/yearly billing toggle** (−15% yearly = a fixed 12-month prepaid term); the `/contract` form gained selectable **add-on steppers** (languages/mailboxes/social) + a billing choice that compute **one canonical monthly total** (drives § 2 line items, buy-out, VAT, the yearly figure, and the contract number) + a **fixed 12-month yearly term** + a **simulated "BAR" agency signature**; the contract-form UI was then modernized (segmented toggle · +/− steppers · social switch · live price summary). 🔴 lawyer review now also covers the yearly term + the simulated signature. Details in `docs/clients/baragency/` (BRIEF.md #7b + the per-client `CLAUDE.md` 2026-06-16 section).

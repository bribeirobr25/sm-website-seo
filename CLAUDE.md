...
- **Portfolio diversity is a hard gate, not a nice-to-have** (added 2026-05-23 after a 6-demo monoculture incident). Before committing `tokens.css` for any new demo/client, audit existing `clients/demo-*/src/styles/tokens.css` per `COLOR.md` §6.5 "Portfolio diversity gate." Specifically: no two demos share a dominant background hue (cream/bone counts as ONE family — at most one demo in 6 may default to cream); at least one demo in any 6-demo portfolio must be dark-bg-dominant; at least one accent must be a saturated pop. Before writing `index.astro`, audit existing `clients/demo-*/src/pages/index.astro` per `DESIGN-BEST-PRACTICES.md` §6.5 "Section composition variation" — if your proposed composition uses ≥ 6 components in the same order as any existing demo, REWORK it. The agency component library has 32 components precisely so each project picks a different subset; defaulting to the universal-9 composition (Hero → preview → About+Stats → Team → Timeline → Testimonial → FAQ → Map → CTA) is a rule violation.
- **Cream / bone / off-white is NOT a default base color** — it's a vertical-specific choice (gastronomy-heritage, fine-dining, boutique-salon-editorial, artisan). Lawyer / gym / dental / real-estate / tech / automotive / dark-register-barber / cool-modern-yoga should NOT default to cream. If the per-vertical template doesn't call out cream, pick a different base. Per `COLOR.md` §6 Anti-patterns #11 + §6.5 audit gate.
...
- **Verify CTA color in a real browser, not just the class string** (added 2026-05-25 after 6-demo invisible-CTA incident). Tailwind v4 with an UNLAYERED `body { color: var(--color-text) }` rule silently shadows every `.text-X` utility — class strings read like "white text on dark bg" but render "dark on dark." Open dev tools, hover a primary CTA, check `getComputedStyle(button).color` matches the intended hex. Every demo's `src/styles/global.css` body rule MUST be inside `@layer base { ... }` — both scaffolds ship the fix. See `DESIGN-BEST-PRACTICES.md` §7 "CTA contrast — Tailwind v4 @layer base requirement" + `CHECKLIST.md` §Accessibility "Browser-inspected CTA color."
- **Never rewrite scaffold infrastructure files from scratch in per-client setup** (added 2026-05-26 after Kodama-bonsai regression incident). The scaffolds at `scaffolds/astro-tier2/` and `scaffolds/nextjs-tier3/` ship a baseline `BaseLayout.astro` + `CookieBanner.astro` + `DemoBanner.astro` + `consent.ts` + `Footer.astro` with: JSON-LD schema emission, BreadcrumbList auto-derivation, hreflang alternate, locale-driven CookieBanner (DE/EN strings via `SITE.i18n[locale].consent`), Manage-Preferences link with `consent:reopen` handler. **These features are non-negotiable agency baseline.** Per-client setup CUSTOMIZES (palette, brand, content) but NEVER REPLACES these files. If a per-client need surfaces (e.g. per-page `Article` schema), use the existing extension points: `extraSchema` prop on BaseLayout, `i18n[locale].consent` block in site.ts. The Kodama-bonsai demo (2026-05-26) was rebuilt with a hand-written BaseLayout that dropped JSON-LD + BreadcrumbList + hreflang + correct OG image path simultaneously — 5 audit blockers from one rewrite. Don't do that.
- **When a per-client build introduces a generally-useful pattern, audit whether it belongs in the scaffold and document it as a recipe** (added 2026-05-26 after Kodama-bonsai 4-locale build closure). Patterns that emerged in one client (e.g. multi-locale extension helpers, new data shape, fresh component, novel measurement convention) should be triaged through three buckets: **BACKPORT NOW** to the matching scaffold if it's generic, low-risk, and useful at the scaffold's default complexity (e.g. `Locale` / `LOCALE_LANG` / `LOCALE_PREFIX` / `LOCALES` helpers — useful even at 2 locales) · **DOCUMENT AS RECIPE** in the matching `docs/design/` file if it's too client-specific to default-include but worth the next agent recalling (e.g. `PAGE_STRINGS` module, `tr()` fallback helper, translation-as-merge data pattern, `[locale]/*.astro` dynamic template) · **DEFER** if not yet proven across multiple clients. Reference the per-client demo as a worked example in the doc. Don't let agency learnings stay locked in a single client folder — the next agent reading the scaffold or the docs should be able to find the pattern without grepping `clients/*`.
- **Demos require real photographs, not gradient placeholders** (added 2026-05-25 after 3-demo "wireframe-not-site" incident). Minimum 5 Unsplash CC0 photos per demo (Hero + 4 supporting). Source recipe in `DESIGN-BEST-PRACTICES.md` §3 "The real-photo rule." Verify each download is a valid JPEG (`file public/img/photo/*.jpg | grep -v JPEG` should output nothing). Hard rule: no demo may ship with zero photographs.

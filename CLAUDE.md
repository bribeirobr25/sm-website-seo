...
**Live agency-self marketing site:** `clients/agency-breno-bar/` deployed at https://agency-breno-bar.vercel.app (Tier 2 Astro · trilingual EN/DE/pt-BR · Apple-inspired register · Resend-wired contact form · 104 static + 3 SSR routes). **Inbound-funnel sprint added 2026-06-04** (borrowed from the `icreateyoursite.com` benchmark at `docs/benchmark/`): published hybrid `/pricing`, paid `/website-check` micro-product, two free lead-tools (`/tools/website-scan` PSI scanner + `/tools/gbp-check`), 24 German `/webdesign-berlin/*` local-SEO pages, home social-proof/promises/FAQ, and WhatsApp/phone wired-but-hidden — concrete values DRAFT. Pre-production: DRAFT legal items + real domain + Resend API key + funnel DRAFTs pending. Per-client docs at `docs/clients/agency-breno-bar/`.

...
|---|---|---|---|
| **Heritage / family restaurant** (owner-operated, multi-decade reputation, comfort cuisine) | Warm earth + terracotta + cream paper | `--color-bg: #f7f0e5` (warm cream)<br>`--color-text: #1f1a14` (warm near-black)<br>`--color-accent: #c2410c` (terracotta)<br>`--color-border: #e5d9c5` | Originally developed during the Porto dos Ribeiros engagement (Astro variants A/B/C, since deprecated). Reads "home cooking" without falling into Tuscan-cliché. Survives food photography that includes red sauces and dark meats. For a current live example, see `clients/demo-restaurant-adele/` (Archetype A heritage). |
...
**The cold-call pattern** — proposing two or three variants to the same prospect so they pick the visual direction — is documented in `SALES.md` §The pitch call. A prior three-variant gastronomy worked example (Porto dos Ribeiros A/B/C, on Astro) was scrapped in 2026-05-16 because it pre-dated the legal/KPI/monitoring rule expansion — both code and per-client docs were deleted; no archived reference remains. For a current Gastronomy worked example, see live builds at `clients/demo-restaurant-adele/` (Archetype A heritage) + `clients/demo-coffee-saltlines/` (Archetype B specialty-café) + `clients/demo-eiscafe-bellini/` (Archetype B).

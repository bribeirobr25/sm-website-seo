...
### Schema auto-emission — what the agency canonical `BaseLayout.astro` ships for free

Added 2026-05-23 from the 3-demo audit batch. The canonical `BaseLayout.astro` (used by every agency Astro Tier 2 client) auto-emits two JSON-LD blocks beyond the always-emitted `Restaurant`/`LocalBusiness` `@graph`:

| Schema | Emitted automatically when… | Source |
|---|---|---|
| **`BreadcrumbList`** | URL path has ≥1 segment after the locale root (skips `/` and `/en/`). Derived from `Astro.url.pathname` + the page `title` prop. Locale-aware home label ("Startseite" / "Home"). | Auto-generated by `deriveBreadcrumbs()` in `BaseLayout.astro` |
| **`FAQPage`** | The canonical `FAQ.astro` component appears on the page with `emitSchema={true}` (the default). Generated from the same `items` array that renders the visible accordion, guaranteeing the schema matches the rendered text exactly (Google's rich-result requirement). | Auto-generated inside `_impl/FAQ.astro` |

**Implication for new client builds:** if you use the canonical scaffold + the canonical `FAQ` component, you get both schemas without writing any extra schema code. Don't re-emit them in `lib/seo/schema.ts` (creates duplicate JSON-LD — Google's structured-data tester will flag).

**When BreadcrumbList does NOT emit:** home pages (`/` and `/en/`) — by design, single-level navigation doesn't justify a breadcrumb. The agency rule (`DESIGN-BEST-PRACTICES.md`): emit `BreadcrumbList` JSON-LD without rendering a visual breadcrumb component, since most builds are flat 3-5 pages and visual breadcrumbs are overkill at that depth.

**Custom breadcrumb names per page** (when the URL slug doesn't read as a human-friendly label): pass an explicit `title` prop to `BaseLayout` — that's what the breadcrumb leaf-name uses. Example: `<BaseLayout title="Karte" />` produces `[Startseite → Karte]` not `[Startseite → /gelato]`.

### Schema `image` URL — must return 200

Audit-driven gate from 2026-05-23. The `image` property inside every `LocalBusiness`/`Restaurant`/`CafeOrCoffeeShop`/etc. `@graph` node MUST resolve to a real image. Google's rich-result tester will flag a 404; AI Overviews + Knowledge Graph won't surface the image at all. Verify before every production cutover:

```bash
curl -sI "${SITE_URL}$(curl -s ${SITE_URL}/ | grep -oE '"image":"[^"]+"' | head -1 | sed 's|.*"image":"||;s|"||')" | head -1
# expect: HTTP/2 200
```

This catch came from a real bug — Saltlines `lib/seo/schema.ts` referenced `hero-surf-coffee.jpg` which didn't exist after a file rename. The image was never broken on the page (it used a different src) but the schema was emitting a 404 URL to crawlers.

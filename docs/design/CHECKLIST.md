...
- [ ] At least one real photo of the physical business used prominently (paying clients) — **OR** for portfolio demos: minimum 5 CC0 photos from Unsplash/Pexels wired into Hero + supporting sections per `DESIGN-BEST-PRACTICES.md` §3 "The real-photo rule" (added 2026-05-25). **No demo may ship with zero photographs** — gradient placeholders + SVG decorations + "photo to follow" captions count as zero.
- [ ] **Photo source verified valid JPEG/WebP** — `file public/img/photo/*.jpg | grep -v JPEG` must output nothing (catches 404-saved-as-jpg failures, see `DESIGN-BEST-PRACTICES.md` §3 Demo asset discipline)

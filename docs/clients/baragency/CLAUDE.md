...
- **Contract `/contract` is now plan-driven and reads from `/pricing` as the single source of truth.** § 2 mirrors the selected plan's FUNNEL feature list verbatim (incl. its "Everything in X, plus:" line); the old `SERVICE_SCOPES` mechanism + 4-checkbox Services step were deleted. The **draft/lawyer red banner was removed** per owner request (the "not lawyer-reviewed" caveat is therefore no longer on-document — still true). Phone field added; dates auto-fill (signature = today, start = +5 days); go-live removed; sticky toolbar fixed to `top-0`. The monthly fee is the single price input (auto-filled from the plan); the one-time buy-out (18× the monthly fee) is a **derived note** under it, not a field, and recomputes live with the fee. Billing is **anniversary-based** (added 2026-06-12): the fee bills on the **start date** each month, not a fixed 1st — § 3/§ 4 use "billing month" anchored to the start date (clamped to the month's last day for the 29th–31st). § 3 also prints the agency **payment details** (bank transfer IBAN/BIC + PayPal, from `SITE.payment`); accepted methods are bank transfer or PayPal. **(2026-06-14)** § 3 now also shows an auto-generated **contract number** — format `BAR-<planCode+price>-<client-slug>-<YYMMDD>`, e.g. `BAR-G390-CAFE-260612` — rendered as "Contract no." in the header and as the **payment reference** in § 3, plus a **Sender (pay as)** line (= client business name) and an explicit **bank-OR-PayPal** ("pay by ONE of…") choice. Built client-side in `contract.astro` from plan + business name + signature date; presentation only, no clause text changed. **(2026-06-16 — add-ons, billing, signature)** The form gained **add-on steppers** (extra languages/mailboxes/social channels) + a **social-media toggle switch** (auto-on + locked "included" for Complete) + a **monthly/yearly billing** segmented toggle. All fold into **one canonical `monthlyTotal`** (audit A2) that drives § 2 add-on line items, the buy-out (18× total), VAT, the yearly figure, AND the contract number — which now encodes the full monthly total (e.g. `BAR-G555-…`); the `price_retainer` field is now **read-only/derived**. **Yearly** = `round(monthlyTotal × 12 × 0.85)`, a **fixed 12-month prepaid term** (§ 4 gains a yearly variant: no refund except Widerrufsrecht; § 3 gains a yearly fee line + discount line); an `s3ReferenceNote` clarifies the reference is the plan ID, not the amount due (a yearly client pays the yearly figure while the ref stays the monthly total). The agency signature column now carries a **simulated "BAR" SVG mark** (`src/lib/signature.ts`, vector/`currentColor`/specimen) + "Berlin, {date}". New numeric data lives in `funnel.ts` (`addons`/`billing`); new clauses in `contract-strings.ts` (en+de). Add-on form controls are `.no-print`; new § 2/§ 3/§ 4 content uses the existing print-safe classes. **🔴 lawyer review now also covers the yearly fixed term + the simulated signature** (BRIEF.md #7b). **(2026-06-17 — form UI modernized)** Step 3 of the contract form now uses a **segmented billing toggle**, **+/− steppers** for the add-on counts, a **social toggle switch**, and a **live price-summary card** (replacing the plain inputs/checkbox + read-only fee field). Field `name`s/values and `render()` are unchanged → the rendered contract + PDF are identical; the form panel stays `.no-print`. **(2026-06-17 — contract footer fix)** Two pre-existing print cosmetics (not regressions from the add-on work) fixed: the footer string dropped its dangling `· Page`/`· Seite` suffix (no `counter(page)` ever rendered a number) in `contract-strings.ts` (en+de), and the footer top margin was trimmed `mt-16`→`mt-8` in both `contract.astro` + `de/contract.astro` so the print-to-PDF no longer spills onto a near-blank trailing page. Verified via headless-Chromium print-to-PDF in both locales (EN 6→5 pages; DE's longer text keeps a 6th page but it is now a clean full signature page, not a blank orphan).

...
Per `DESIGN-BEST-PRACTICES.md` §5 "Sourcing the palette," every per-client palette must declare which priority tier it was sourced from. Porto is built as **three variants** with different palettes per variant — the source tier is declared per variant:

| Variant | Source tier | Notes |
|---|---|---|
| **A — Stripped editorial (this codebase, `clients/porto-dos-ribeiros/`)** | **Tier 3 — Sampled from existing client website + tier 5 vertical-default merge** | `portodosribeiros.com` already had a warm-clay + cream visual identity; sampled the dominant background and CTA tones. Cross-referenced with `templates/gastronomy.md` §6 "Default palette when the client has no brand" → **Heritage / family restaurant** sub-archetype, which is **anchored to this build** (the template default literally cites this codebase as the reference implementation). The 4 declared keys (`bg` / `text` / `accent` / `border`) match the template default by definition. Extra agency-specific tokens: `--color-open` / `--color-open-deep` (herb-green for "Aberto" badge) and `--color-azulejo` / `--color-azulejo-light` (place-identity blue) — additions, not contradictions. |
| **B — Modern conversion-first (`clients/porto-dos-ribeiros-v2/`)** | **Tier 5 — Vertical-default palette** per `templates/gastronomy.md` §6 → **Modern conversion-first** sub-archetype (Sweetgreen-derived cream + sage + bright lime CTA) | No client signal. V2 implements the Sweetgreen-style archetype as a deliberate alternative direction for the cold-call comparison. **Hex values diverge from the template default starting points** (V2 predates the template; the family designation is correct but token values are V2's own implementation). |
| **C — Heritage editorial (`clients/porto-dos-ribeiros-v3-heritage/`)** | **Tier 5 — Vertical-default palette** per `templates/gastronomy.md` §6 → **Premium editorial / heritage storytelling** sub-archetype (deep cocoa + parchment + saffron) | No client signal. V3 implements the Dishoom-style archetype as a deliberate alternative direction. **Hex values diverge from the template default starting points** (same reason as V2 — V3 predates the template). |

> **Token-name inconsistency (pre-existing, flagged by 2026-05-14 audit):** Variant A's herb-green is documented above as `--color-secondary` (and elsewhere in this file) but the actual token in `clients/porto-dos-ribeiros/src/styles/tokens.css` is `--color-open` (same color value `#3F6B3A`). The names disagree; the value is correct. **Decide which is canonical (`--color-open` is in-use across components — keep that and update `design.md`) and reconcile.** See `docs/audit/porto-dos-ribeiros-2026-05-14.md` §4 Phase B for the action item.

**Why three sources, not one:** the three-variant cold-call strategy (per `BRIEF.md` and `templates/gastronomy.md` §9.4) requires three distinct palettes. The client picks one during the call; the others are agency-internal portfolio pieces only. Once the owner picks a variant, the other two collapse and Porto's docs revert to a single-palette source line.

**Override condition:** if the owner provides a brand guide or existing identity materials during the cold call, re-source from tier 1 or 2 and update tokens accordingly for the chosen variant.

...
export interface ContractStrings {
  ...
  s3Heading: string;
  s3BuildFeeLine: (eur: string) => string;
  s3BuildFeeSplit: string;
  s3RetainerLine: (eur: string) => string;
  s3PaymentTerms: string;
  s3PaymentDefault: string;
  s3VatKleinLine: string;
  s3VatRegularLine: (netEur: string, vatEur: string, grossEur: string) => string;

  s4Heading: string;
  s4Start: (date: string) => string;
  ...
  signatureBlock: (party: string, role: string) => string;
...
export const CONTRACT_STRINGS: Record<LocaleEnDe, ContractStrings> = {
  en: {
    ...
    s3VatRegularLine: (net, vat, gross) =>
    ...
    s4Start: (date) => `This Agreement begins on ${date} ("Start Date").`,
    s4MinimumTerm:
    ...
    signatureBlock: (party, role) => `${party} (${role})`,
  ...
  de: {
    ...
    s4Start: (date) => `Dieser Vertrag beginnt am ${date} („Vertragsbeginn").`,
    s4MinimumTerm:
